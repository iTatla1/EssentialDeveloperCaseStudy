...
//
//  Created by Muhammad Usman Tatla on 12/6/21.
//
...
class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGetFeedResult_matchesFixedAccountTestData() {
        ...
        case .success(let imageFeed):
            XCTAssertEqual(imageFeed.count, 8, "Expected 8 images in test account")
        ...
        case .failure(let error):
            XCTFail("Expected successful result with feed images got \(error) instead")
            
        default:
            XCTFail("Expected successful result with feed images got no result instead")
        }
    }
